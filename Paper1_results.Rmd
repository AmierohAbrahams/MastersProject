...
transect.pixel.isobath <- function(site, distance = 25000, isobath = 999){ # Only at a distance of 25000km
  # Extract coordinates
  coords <- data.frame(lon = site$lon, lat = site$lat)
  ...
  pixels2 <- pixels[pixels$depth > -999,] # This may remove the only row of data
  if(nrow(pixels2) < 1){
...
for(i in 1:length(west$site)){
  ...
  site_pixel <- transect.pixel.isobath(site, 25000, -900)
...
# Upwelling correlated with wind patterns
## Loading in the Wind data
...
library(tidyverse)
library(circular)
wind_data <- read_csv("wind_data.csv", col_types = cols(X1 = col_number(), date = col_character(), dir = col_number(), hour = col_number(),  speed = col_number()))

wind_daily <- wind_data %>% 
  mutate(date = as.Date(date))%>%
   group_by(temp_sites, date) %>%
   summarise(dir_circ = round(mean.circular(circular(dir, units = "degrees")),2),
