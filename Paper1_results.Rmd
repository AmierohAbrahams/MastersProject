...
save(site_list_sub, file = "Data_P1/site_list_sub")
...
load("Data_P1/SACTN_overlap.RData") # Loading the SACTN_overlap dataset which allows me to match it to the OISST data 
...
match_index <- knnx.index(data = as.matrix(unique_pixel[,1:2]),
                      query = as.matrix(SACTN_overlap[,5:6]), k = 1)

# Select SST pixels nearest to insitu sites
pixel_match <- unique_pixel[match_index,] %>%
  unite(col = combi, lon, lat, sep = "/", remove = F) %>%
  mutate(site = SACTN_overlap$site)
# save(pixel_match, file = "Data_P1/pixel_match.RData")
load("Data_P1/pixel_match.RData")
...
load("Data_P1/OISST_match.RData")
