...
The aim of this is to find the temperature collected at different distances from the coast (distances = 15km, 25km, 35km and 50km).
...
for(i in 1:length(west$site)){
  ...
  site_pixel <- transect.pixel(site, c(10000, 20000, 30000, 40000, 50000)) # RWS: The desired distances
...
sa_lats <- c(-37, -27); sa_lons <- c(14, 50)
...
site_map <- ggplot(data = south_africa_coast, aes(x = lon, y = lat)) + bw_update +
  ...
  scale_fill_viridis(
    ...
    name = "Temperature (°C)",
    alpha = 0.8) +
  #scale_fill_viridis(limits = limits, expression(paste("Temperature (°C)"))) +
  xlab("") + ylab("") +
  annotate("text", label = "INDIAN\nOCEAN", x = 32.00, y = -35.0,
    ...
    annotate("text", label = "Benguela", x = 16.0, y = -31.7,
             size = 6, angle = 302, colour = "black") +
    annotate("text", label = "Agulhas", x = 31.7, y = -31.7,
             size = 6, angle = 50, colour = "black") +
...
Do the in situ and satelllite derived data yield the same number of events. 
...
for(i in 1:length(levels(SACTN_fyears$site))) {
  ...
  y <- subset(OISST_fyears, site == levels(OISST_fyears$site)[i])
  x <- x[x$date %in% y$date,]
  y <- y[y$date %in% x$date,]
  z <- data.frame(site = as.character(x$site[1]), R2 = coef(lm(y$temp~x$temp))[2],na.action=na.exclude)
  R2 <- rbind(R2, z)
}
```
