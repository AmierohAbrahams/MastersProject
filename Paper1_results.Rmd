...
## This is only for the SACTN data
...
# See graph: ("Figures/upwelling_indeces.png")
# Sea Point appears to have missing data
# Lamberts bay appears to have missing data as well
# Can we plot the graph where it ignores the missing data without interpolations? [AJ suggested]
# Plot the temperature anomalies on the same graphs as the upwelling indeces for each of the sites
```
######################################

# Question: How does the intensity and duration of an upwelling event vary at different distances from the coastline???
  # Got the various pixels : load(Data_P1/site_pixels.RData)
  # How do i determine where upwelling occured? - What is the threshold of an upwelling event? # How do i determine the threshold of this event
  # Calculate event count, duration and mean intensity for each pixel
  
Look at the percentile level that seawater temperature must decrease below when an upwelling event occurs
  - To what exttent the temperature of upwelled water  is colder than the mean SST. 
  - How the minumum temperature of upwelled water becomes to be that temperature
  - Why temperature does not become any colder

To do:
- how count, duration, intensity of upwelling changed between 1990s and current times
