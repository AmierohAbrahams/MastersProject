...
The aim of this script is to extract the chlorophyll a netCDF files and convert it to CSV. The chlorophylla data was obtained from MODIS Aqua. The aim of working with this chlorophyll a data is to analyse how it changes/varies with upwelling.
...
#   return(monthlyData)
# }
#  daily2monthly(MODIS_Chloro)

# Pixels and matching
# match_index <- knnx.index(data = as.matrix(daily_chloro[,4:5]),
#                           query = as.matrix(combined_US[,5:6]), k = 1)
# 
# # # Select SST pixels nearest to insitu sites
# pixel_match <- daily_chloro[match_index,] %>%
#   unite(col = combi, lon, lat, sep = "/", remove = F) %>%
#   unite(col = day, year, month, sep = "-", remove = F) %>% 
#   mutate(site = combined_US$site)
# 
# daily_chloro_match <- daily_chloro %>%
#   unite(col = combi, lon, lat, sep = "/", remove = F) %>%
#   unite(col = day, year, month, sep = "-", remove = F) %>% 
