...
The first few lines of this script only looks at a time series of 10 years or greater. However, from line 355 a longer time series is used. This time series has a length of greater than 30 years and so only a few sites are used. With a longer time series I am able to detect for extreme events such as Marine Cold Spells (MCSs). 
...
MCS_SACTN_PN <- event_line(res, spread = 100, metric = "intensity_cumulative",
...
ts <- ts2clm(MUR_SST_DI, climatologyPeriod = c("2002-06-01", "2015-12-30"), pctile = 10)
res <- detect_event(ts, coldSpells = TRUE)
MCS <- res$climatology
MCS_MUR_DI <- event_line(res, spread = 500, metric = "intensity_cumulative",
           start_date = "2011-12-31", end_date = "2014-10-30") +
  ggtitle("MUR_DI")
...
ts <- ts2clm(MUR_SST_HB, climatologyPeriod = c("2002-06-01", "2015-12-30"), pctile = 10)
res <- detect_event(ts, coldSpells = TRUE)
MCS <- res$climatology
MCS_MUR_HB <- event_line(res, spread = 500, metric = "intensity_cumulative",
           start_date = "2011-12-31", end_date = "2014-10-30") +
  ggtitle("MUR_HB")
...
combined_MUR <- ggarrange(MCS_MUR_PN, MCS_MUR_LB, MCS_MUR_O, MCS_MUR_HB, MCS_MUR_SP, MCS_MUR_Kom, MCS_MUR_DI, MCS_MUR_SB, MCS_MUR_YZ, ncol = 3, nrow = 3)
```
...
MCS_g1sst_PN <- event_line(res, spread = 500, metric = "intensity_cumulative",
           start_date = "1991-09-01", end_date = "1992-12-30") +
  ggtitle("G1SST_PN")
...
G1SSTsub_SST_HB <- G1SSTsub_SST %>% 
  filter(site == "Hout Bay") %>% 
  dplyr::rename(temp = nearest1) %>% 
  select(date, temp) %>% 
  dplyr::rename(t = date)

ts <- ts2clm(G1SSTsub_SST_HB, climatologyPeriod = c("1991-09-01", "2015-12-30"), pctile = 10)
res <- detect_event(ts, coldSpells = TRUE)
MCS <- res$climatology
MCS_g1sst_HB <- event_line(res, spread = 500, metric = "intensity_cumulative",
           start_date = "1991-09-01", end_date = "1992-12-30") +
  ggtitle("G1SST_HB")
...
G1SSTsub_SST_DI <- G1SSTsub_SST %>% 
  filter(site == "Dassen Island") %>% 
  dplyr::rename(temp = nearest1) %>% 
  select(date, temp) %>% 
  dplyr::rename(t = date)

ts <- ts2clm(G1SSTsub_SST_DI, climatologyPeriod = c("1991-09-01", "2015-12-30"), pctile = 10)
res <- detect_event(ts, coldSpells = TRUE)
MCS <- res$climatology
MCS_g1sst_DI <- event_line(res, spread = 500, metric = "intensity_cumulative",
           start_date = "1991-09-01", end_date = "1992-12-30") +
  ggtitle("G1SST_DI")
...
combined_G1SST <- ggarrange(MCS_g1sst_PN, MCS_g1sst_LB, MCS_g1sst_O, MCS_g1sst_SP, MCS_g1sst_Kom, MCS_g1sst_DI, MCS_g1sst_YZ, MCS_g1sst_SB, MCS_g1sst_HB, ncol = 3, nrow = 3)
```
...
res <- detect_event(ts, coldSpells = TRUE)
MCS <- res$climatology
MCS_OISST_PN <- event_line(res, spread = 500, metric = "intensity_cumulative",
           start_date = "2016-09-01", end_date = "2018-06-30") +
  ggtitle("OISST_PN")
...
res <- detect_event(ts, coldSpells = TRUE)
MCS <- res$climatology
MCS_OISST_LB <- event_line(res, spread = 500, metric = "intensity_cumulative",
           start_date = "2016-09-01", end_date = "2018-06-30") +
  ggtitle("OISST_LB")
...
OISST_DI <- OISST_sites %>% 
  filter(site == "Dassen Island") %>% 
  select(date, temp) %>% 
  dplyr::rename(t = date)

ts <- ts2clm(OISST_DI, climatologyPeriod = c("1981-09-01", "2018-06-30"), pctile = 10)
res <- detect_event(ts, coldSpells = TRUE)
MCS <- res$climatology
MCS_OISST_DI <- event_line(res, spread = 500, metric = "intensity_cumulative",
           start_date = "2016-09-01", end_date = "2018-06-30") +
  ggtitle("OISST_DI")
...
res <- detect_event(ts, coldSpells = TRUE)
MCS <- res$climatology
MCS_OISST_O <- event_line(res, spread = 500, metric = "intensity_cumulative",
           start_date = "2016-09-01", end_date = "2018-06-30") +
  ggtitle("OISST_O")

OISST_HB <- OISST_sites %>% 
  filter(site == "Hout Bay") %>% 
  select(date, temp) %>% 
  dplyr::rename(t = date)

ts <- ts2clm(OISST_HB, climatologyPeriod = c("1981-09-01", "2018-06-30"), pctile = 10)
res <- detect_event(ts, coldSpells = TRUE)
MCS <- res$climatology
MCS_OISST_HB <- event_line(res, spread = 500, metric = "intensity_cumulative",
           start_date = "2016-09-01", end_date = "2018-06-30") +
  ggtitle("OISST_HB")

combined_OISST <- ggarrange(MCS_OISST_PN, MCS_OISST_LB, MCS_OISST_SB, MCS_OISST_YZ, MCS_OISST_O, MCS_OISST_HB, MCS_OISST_Kom,MCS_OISST_SP, MCS_OISST_DI)
```
