...
This code is testing out the wavelet analyses for the in situ temperature data

```{r}
SACTN_wavelet <- SACTN_daily_temps_indexsplit %>% 
  filter(year(date) %in% seq(2015, 2016))
  
SACTN_wavelet.d <- SACTN_wavelet %>% 
  dplyr::group_by(site, date) %>% 
  dplyr::summarise(temp = mean(insitu_temp, na.rm = TRUE)) %>% 
  dplyr::mutate(no = seq(1:n())) %>%
  dplyr::ungroup() %>% 
  dplyr::select(site, no, temp, date)

ggplot(data = SACTN_wavelet, aes(x = date, y = insitu_temp)) +
  geom_hline(aes(yintercept = mean(insitu_temp)), colour = "salmon") +
  geom_line() +
  facet_wrap(~ site, nrow = 4) +
  theme_bw()

auto.arima(dplyr::filter(SACTN_wavelet, site == "Port Nolloth")[, "insitu_temp"], max.p = 3, max.q = 3, stationary = FALSE, 
           seasonal = FALSE)
