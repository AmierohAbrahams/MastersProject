...
setwd("~/Documents/MastersProject")
...
upwelling_SACTN_daily_wc <- SACTN_daily_wc[-6,] # Here we had two rows of data for Paternoster; i removed the DEA row: All data here are now collected by SAWS 
upwelling_SACTN_daily_wc <- upwelling_SACTN_daily_wc[-10:-14,] # Here i remove the sites that does not fall within the upwelling cell

SACTN_daily_temps <- SACTN_daily_v4.2 %>% 
  left_join(site_list[,c(4,13)], by = "index") %>% 
  filter(index %in% upwelling_SACTN_daily_wc$index)
...
```{r}
SACTN_daily_temps_monthly <- SACTN_daily_temps %>% 
...
Here I load the AVHRR CSV dataset. Large dataset and slows down the laptop. Here i split the data into xaa and xab. Here xaa and xab only include temperature collected for two years.
```{r}
...
# save(combined_OISST, file = "data/combined_OISST.RData")
```
...
```{r}
matched_SST <- OISST_sites %>% 
  left_join(SACTN_daily_temps_indexsplit, by = c("site", "date")) %>% 
    na.trim()
```
...
hee1 <- try1 %>% 
  ...
  dplyr::ungroup() %>% 
  dplyr::select(site, temp, date)


ggplot(data = thee1, aes(x = date, y = temp)) +
