...
wind_daily <- wind_data %>%
  ...
  group_by(site, date) %>%
  summarise(dir_circ = mean.circular(circular(dir, units = "degrees")),
            speed_circ = mean.circular(circular(speed, units = "degrees")))
...
## Load in the sea surface temperatue datasets
...
temp_wind_SACTN <- temp_chloro(df = SACTN_fyears)
temp_wind_OISST <- temp_chloro(df = OISST_fyears)
temp_wind_G1SST <- temp_chloro(df = G1SST_fyears)
temp_wind_K10 <- temp_chloro(df = K10_fyears)
```


- Create plots shwoing predominant wind direction during an upwelling evnt
- Create wavelet analyses
