...
Creating a seasonal column. Allowing me to compare the intensity/duration of upwelling on a seasonal basis. This should to be repeated for each of the sites within each of the datasets (SACTN, MUR, OISST, G1SST and K10). The code bellow needs to be edited. Here I only focused on Port Nolloth within the SACTN dataset.  
...
combined_seasonal_2014 <- combined_seasonal(df = PN_14)
combined_seasonal_2015 <- combined_seasonal(df = PN_15)
combined_PortNolloth <- ggarrange(combined_seasonal_2014, combined_seasonal_2015)

# ggsave(plot = combined_PortNolloth, filename = "Figures/combined_PortNolloth.png")
```

Creating a wavelet analyses for the year 2015. This code creates a plot per season.

```{r}
seasonal_PN_15 <- PN_15 %>% 
  dplyr::rename(temp = insitu_temp)
...
temp.d <- temp.d(df = seasonal_PN_15)
...
PN_wave <- dlply(temp.d, .(site, season), wl.fun)
