...
Working with remotely-sensed SST and *in-situ* collected coastal seawater temperature, in the analyses bellow I aim to show how the intensity and duration of upwelling events varies. It is important to remember that each of the datasets used within this study obtained SSTs at different resolutions. Here i was only required to plot a time series of 1year.
...
```{r}
one_year <- function(df){
  upwelling<- df %>% 
  filter(year(date) %in% seq(2014, 2015)) %>% 
  drop_na()
}

SACTN_1yr_US1 <- one_year(df = overlap_ts) 
MUR_1yr_US1 <- one_year(df = insitu_MUR) 
OISST_1yr_US1 <- one_year(df = insitu_OISST) 
G1SST_1yr_US1 <- one_year(df = insitu_G1SST) 
K10_1yr_US1 <- one_year(df = insitu_K10) 
```
...
o
temp.d <- temp.d(df = overlap_ts) 
# temp.d <- temp.d(df = insitu_MUR) 
# temp.d <- temp.d(df = insitu_OISST) 
# temp.d <- temp.d(df = insitu_G1SST) 
# temp.d <- temp.d(df = insitu_K10) 
